...
require_once('db_fns_userauth.php');
...
function register_user($username, $password, $confpass) {
    ...
    $query = "insert into users values
            ('" . $conn->real_escape_string($username) ."',sha2('". $conn->real_escape_string($password)."', 224),0)";
...
// else return false
  ...
  $result = $conn->query("select * from users
                         where username='". $conn->real_escape_string($username)."'
                         and password = sha2('". $conn->real_escape_string($password)."', 224)
						             and hasadmin = 1");
  if (!$result) {
     return 0;
  }
...
// return true or false
  ...
  if (login($username, $old_password)) {
    ...
    $result = $conn->query("update users
                            set password = sha2('". $conn->real_escape_string($new_password)."', 224)
                            where username = '". $conn->real_escape_string($username) ."'");
    if (!$result) {
